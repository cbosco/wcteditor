...
(function($) {
	...
	function _loadTemplates() {		
		...
		templates['linkModal'] = '<div class="wcte-linkModal wcte-modal"><label>URL:<input type="text" value="${href}"/></label><button>OK</button><a>Cancel</a></div>';
		templates['linkOverlay'] = '<div class="wcte-linkOverlay wcte-modal" contenteditable="false">${url}<br/><a href="${url}" target="_blank">Open link</a></div>';
		templates['spellCheck'] = '<div class="wcte-spellCheckModal wcte-modal" contenteditable="false">${suggestions}<a>Ignore</a></div>';
		templates['spellCheckSuggestion'] = '<a class="wcte-sug">${value}</a><br/>';
		templates['contents'] = '<span class="wcte-placeholder">${placeholderText}</span> ';
		
		templates['wcteditor'] = function(that) {
	...
	function _init() {
		...
		if (that.supportsContentEditable()) {	
			...
			that.textarea.after($(templates.wcteditor(that))).hide();
			...
			if (that.showCharCount) {
				that.container.append('<div class="wcte-charCount">' + that.charCountTmpl.replace('${chars}', that.chars) + '</div>');
			...
			.blur(function(){
				if (that.editor.text().length < 1 && !that.editor.find("span.wcte-placeholder")[0])
					that.editor.prepend($(templates.contents.replace('${placeholderText}',that.placeholderText)));
			})
	...
	function _setLink(leftPosition) {
		...
		that.container.append($(templates.linkModal.replace('${href}', href || '')))
		...
		if (document.body.createTextRange) {
			...
			.delegate("font","click",function() {
				...
				$.each(suggestions, function(i, value) {
					if (value) {
						suggestionsHTML += templates.spellCheckSuggestion.replace('${value}', value);
					}
				});
				that.editor.after($(templates.spellCheck.replace('${suggestions}', suggestionsHTML)));
